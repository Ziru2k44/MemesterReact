...
import "./Albuns.css";
import { getAlbuns, getMoreAlbuns } from "../../util/APIUtils";
...
class Albuns extends Component {
  ...
  nextlist() {
    this.setState({loading:true,});
    getMoreAlbuns(this.state.idItems,this.state.nextItems,"after")
    ...
    .then(response => {
      ...
      this.setState({
        ...
        listItems: response.data.map(album => (
  ...
  previouslist() {
    ...
    this.setState({loading:true});
    getMoreAlbuns(this.state.idItems,this.state.previousItems,"before")
